...
class FreightService
{
     ...
     * @return JsonResponse
     */
    public function updateFreight(string $uuid, array $data): JsonResponse
    {
        DB::beginTransaction();
        try {
            $freight = Freight::where('uuid', $uuid)->firstOrFail();

            // Check if coordinates exist in the update data and format them
            if (isset($data['origin_coord'])) {
                $originCoords = explode(',', $data['origin_coord']);
                $data['origin_coord'] = DB::raw("ST_PointFromText('POINT(" . trim($originCoords[0]) . " " . trim($originCoords[1]) . ")')");
            }
            ...
            // `required_equipment` is cast to array in model, so pass it directly.
            if (isset($data['required_equipment']) && is_string($data['required_equipment'])) {
                 $data['required_equipment'] = [$data['required_equipment']];
            }
            ...
            DB::rollBack();
            return $this->errorResponse('Failed to update freight: ' . $e->getMessage(), 500);
        }
    }
