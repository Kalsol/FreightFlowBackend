...
class RegisterUserAction
{
    ...
    public function execute(array $data): JsonResponse
    {
        ...
        $key = 'registration_attempt:' . $contactNumber;
        ...
        try {
            ...
            $this->otpService->storeOtpInDatabase($user->contact_number, self::OTP_REASON_REGISTRATION);
            ...
            return $this->successResponse(
                $user->only(['id', 'contact_number']), // Return only relevant user data
                'Your account has been created. A verification code has been sent to your phone.',
                201 // 201 Created
            );
        } catch (\Throwable $e) {
            DB::rollBack(); // Rollback the transaction on error
            Log::error("User registration failed for contact number {$contactNumber}: " . $e->getMessage(), [
                'exception' => $e,
                'contact_number' => $contactNumber,
            ]);
